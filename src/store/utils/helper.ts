  ...
  (store: T) => {
    if (import.meta.env.DEV && window[`store_${name}`]) {
      import.meta.hot?.send(
        'message',
        'The store has been changed, reloading...',
      )
  ...
  async run(): Promise<void> {
    ...
    if (this.optimisticExecutor) {
      try {
        await Promise.resolve(
          this.optimisticExecutor(this._snapshot, this._ctx),
        )
      } catch (error) {
    ...
    if (this.onPersist) {
      await runTransactionInScope(() =>
        Promise.resolve(this.onPersist!(this._snapshot, this._ctx)),
      )
    }
  }
}

export const createTransaction = <S, Ctx>(
  snapshot?: S,
  ctx?: Ctx,
): Transaction<S, Ctx> => {
